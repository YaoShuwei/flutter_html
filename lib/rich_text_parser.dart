...
class LinkTextSpan extends TextSpan {
  ...
  LinkTextSpan({TextStyle style, this.url, String text, OnLinkTap onLinkTap, List<TextSpan> children})
      : super(
          style: style,
          text: text,
...
class HtmlRichTextParser extends StatelessWidget {
  ...
  static get _supportedElements => List()..addAll(_supportedStyleElements)..addAll(_supportedSpecialtyElements)..addAll(_supportedBlockElements);
  ...
  Widget build(BuildContext context) {
    ...
    widgetList.forEach((dynamic w) {
      if (w is BlockText) {
        ...
        if ((childTextSpan.text == null || childTextSpan.text.isEmpty) && (childTextSpan.children == null || childTextSpan.children.isEmpty)) return;
  ...
  void _parseNode(dom.Node node, ParseContext parseContext, BuildContext buildContext) {
    ...
    if (node is dom.Text) {
      ...
      if (parseContext.condenseWhitespace) {
        ...
        else if (parseContext.parentElement is TextSpan || parseContext.parentElement is LinkTextSpan) {
      ...
      if (parseContext.parentElement == null) {
        ...
        if (treatLikeBlock) {
          ...
          BlockText blockText = BlockText(
            margin: EdgeInsets.only(top: 8.0, bottom: 8.0, left: parseContext.indentLevel * indentSize),
      ...
      // handle specialty elements
        ...
        switch (node.localName) {
          ...
          case "table":
            ...
            nextContext.rootWidgetList.add(Container(margin: EdgeInsets.symmetric(vertical: 12.0), child: nextContext.parentElement));
            break;
          ...
          case "th":
            ...
            nextContext.childStyle = nextContext.childStyle.merge(TextStyle(fontWeight: (node.localName == 'th') ? FontWeight.bold : FontWeight.normal));
          ...
          case "caption":
            ...
            RichText text = RichText(textAlign: TextAlign.center, textScaleFactor: 1.2, text: TextSpan(text: '', children: <TextSpan>[]));
            Expanded cell = Expanded(
        ...
        if (customTextStyle != null) {
          final TextStyle customStyle = customTextStyle(node, nextContext.childStyle);
          if (customStyle != null) {
                ...
                if (node.attributes['src'].startsWith("data:image") && node.attributes['src'].contains("base64,")) {
                  ...
                  parseContext.rootWidgetList.add(GestureDetector(
                    child: Image.memory(
                      ...
                      width: imageProperties?.width ?? ((node.attributes['width'] != null) ? double.tryParse(node.attributes['width']) : null),
                      height: imageProperties?.height ?? ((node.attributes['height'] != null) ? double.tryParse(node.attributes['height']) : null),
...
//                    ),
          ...
          default:
            ...
            BlockText blockText = BlockText(
              margin: node.localName != 'body' ? _customEdgeInsets ?? EdgeInsets.only(top: 8.0, bottom: 8.0, left: parseContext.indentLevel * indentSize) : EdgeInsets.zero,
              padding: EdgeInsets.all(2.0),
              decoration: decoration,
              child: RichText(
                textAlign: textAlign,
